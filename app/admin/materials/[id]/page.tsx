...
export default function EditMaterialPage() {
  ...
  const materialId = params.id as string // Ambil ID dari URL
    ...
    const fetchMaterial = async () => {
      ...
      if (error) {
        ...
        if (data.content && Array.isArray(data.content)) {
          setSections(data.content as MaterialSection[])
        }
      }
      setFetching(false)
    }
  ...
  // --- HANDLER INPUT (SAMA SEPERTI HALAMAN BARU) ---
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
    const { name, value } = e.target
    setFormData(prev => ({ ...prev, [name]: value }))
  }

  const addSection = () => setSections([...sections, { title: '', content: '', examples: [''] }])
    ...
    try {
      ...
      const { error } = await supabase
        ...
        .update({
            ...
            content: sections, // Update JSON baru
            topics_count: sections.length
        })
        .eq('id', materialId) // KUNCI: Update HANYA ID ini

      if (error) throw error

      alert("Perubahan berhasil disimpan!")
      router.push('/admin/materials') // Balik ke list
  ...
  if (fetching) {
    return (
      <div className="min-h-screen bg-slate-950 flex items-center justify-center text-white">
        <Loader2 className="w-8 h-8 animate-spin mr-2" /> Memuat data materi...
      </div>
    )
  }
    ...
    <div className="min-h-screen bg-slate-950 text-white p-4 md:p-8 font-sans">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex items-center gap-4 mb-8">
          ...
          <div>
            <h1 className="text-3xl font-bold text-white">Edit Materi</h1>
            <p className="text-slate-400">Ubah detail atau konten materi pembelajaran.</p>
          </div>
        </div>
